...
public class OAuthClient {
    ...
    public TokenEntry fetchToken(OAuthProfile profile) throws Exception {
        return switch (profile.getGrantType()) {
            case CLIENT_CREDENTIALS   -> clientCredentials(profile);
            case RESOURCE_OWNER_PASSWORD -> resourceOwnerPassword(profile);
        };
    }
    ...
    private TokenEntry resourceOwnerPassword(OAuthProfile profile) throws Exception {
        StringBuilder body = new StringBuilder("grant_type=password");
        body.append("&username=").append(enc(profile.getRopcUsername()));
        body.append("&password=").append(enc(profile.getRopcPassword()));
        appendScopes(body, profile);
        appendClientAuth(body, profile);
        api.logging().logToOutput("[OAuth Helper] ROPC token request for: " + profile.getName());
        return post(profile, body.toString());
    }
    ...
    private TokenEntry post(OAuthProfile profile, String formBody) throws Exception {
        ...
        String httpVer = (profile.getHttpVersion() != null && !profile.getHttpVersion().isBlank())
                ? profile.getHttpVersion() : "HTTP/1.1";
        raw.append("POST ").append(path).append(" ").append(httpVer).append("\r\n");
